---
title: "@anvia/gemini: Usage Patterns"
description: "Common ways to compose @anvia/gemini with adjacent Anvia packages."
section: packages
sidebar:
  group: "@anvia/gemini"
  order: 3
...
@anvia/gemini owns the provider adapter. It should be created at the model boundary, then passed into `@anvia/core` as a completion, embedding, transcription, audio, image, OCR, or listing model where that package supports it.
...
Do construct `GeminiClient` once per runtime boundary or request scope. Do record selected provider and model in logs or traces. Do pass provider-specific options through the model request only when the behavior is intentionally provider-specific.
