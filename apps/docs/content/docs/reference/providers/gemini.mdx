...
Purpose: factory for Gemini API or Vertex AI-backed completion, embedding, image generation, and transcription models.

Return behavior: creates or uses a `GoogleGenAI` client, then returns Gemini completion and embedding models.
