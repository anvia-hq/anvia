...
NIM exposes `GET /v1/models` for loaded and available inference models.
...
const response = await fetch(`${baseUrl}/models`, {
  headers: {
    Authorization: `Bearer ${process.env.NVIDIA_NIM_API_KEY ?? "local"}`,
  },
});

if (!response.ok) {
  throw new Error(`NVIDIA NIM models request failed: ${response.status}`);
}
