...
The endpoint must return an Anvia stream. If you are building it from scratch, start with
...
Use controlled props when the draft, selected entities, or attachments need to be mirrored into app
state.
...
For a full trigger guide, see [Composer triggers](/docs/react-ui/composer-triggers). Use
...
`useChat(...)` as shown in [Request shape](/docs/react-ui/request-shape).
...
For file handling across the route boundary, see [Attachments end to end](/docs/react-ui/attachments-end-to-end).
