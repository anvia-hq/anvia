...
If your Ollama Cloud account exposes the OpenAI-compatible models endpoint, use `/v1/models`.
...
if (!response.ok) {
  throw new Error(`Ollama Cloud models request failed: ${response.status}`);
}
