...
Use `@anvia/core/loaders` when ingestion starts from local text files or PDFs.
...
Anvia loaders do ingestion only. For text chunking beyond PDF pages, preprocess text in application code before calling `embedDocuments(...)`.
