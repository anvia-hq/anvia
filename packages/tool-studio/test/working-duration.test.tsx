...
describe("WorkingDuration", () => {
  ...
  it("formats nonnegative elapsed time as minutes and seconds", () => {
    expect(formatWorkingDuration(-1)).toBe("Working - 0m 0s");
    expect(formatWorkingDuration(Number.NaN)).toBe("Working - 0m 0s");
    expect(formatWorkingDuration(999)).toBe("Working - 0m 0s");
  ...
  it("ticks while active and freezes at the supplied duration", () => {
    ...
    expect(container.textContent).toBe("Working - 0m 0s");
    ...
    expect(container.textContent).toBe("Working - 1m 5s");
  });
});
